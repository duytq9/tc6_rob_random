...
//******************************************************************************
  ...
  class rand_constr_seq extends rob_seq_base;
    ...
    // -----------------------------------------------------------------------
    bit        tag_alive [20];      // 1 = bit đang được allocated
    bit [6:0]  tag_owner [20];      // rob_idx của branch chiếm bit
    ...
    task body();
      ...
      // Init state
      foreach (tag_alive[i]) tag_alive[i] = 0;
      foreach (tag_owner[i]) tag_owner[i] = 0;
      current_br_mask = 0;
      ftq_counter = 0;
    ...
    function int pick_action();
      ...
      // Không có branch tag alive → cấm resolve / mispredict
      if (n_alive_br == 0) begin wm = 0; wr = 0; end

      // Cap branch saturation: nếu sắp cạn 20 bit → ép resolve / mispredict
    ...
    //              Constraint #4: chỉ chọn bit từ pool tag_alive.
    // ========================================================================
    task do_resolve();
      rob_transaction tr;
      int             alive_bits[$];
    ...
    // ========================================================================
    //  do_mispredict — mispredict 1 bit ALIVE; kill entries phụ thuộc;
    ...
    task do_mispredict();
      ...
      // Free chính bit_pos (branch đã được resolve)
      tag_alive[bit_pos] = 1'b0;
      current_br_mask  &= ~mask;

      // Mark branch instruction đã resolved trong tracking (vẫn alive,
      // chờ WB & commit, nhưng không còn là branch chưa-resolve nữa)
      foreach (live_q[i]) begin
        if (live_q[i].is_br && live_q[i].br_tag_bit == bit_pos) begin
          live_q[i].is_br      = 1'b0;
          live_q[i].br_tag_bit = -1;
