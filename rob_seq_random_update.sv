...
//      → Tracking thêm flush_wait_cycles, re-arm = 4 mỗi lần
//        flush_valid được detect (1 cycle margin trên 3 cycle HW).
...
//******************************************************************************
  ...
  class rand_constr_seq extends rob_seq_base;
    ...
    //  dùng tail SAI.
    //  → Re-arm flush_wait_cycles = 4 (1 cycle margin trên 3 cycle hardware).
    ...
    function void update_after_dut_events();
      ...
      if (vif.mon_cb.flush_valid) begin
        ...
        foreach (tag_wb_done[i]) tag_wb_done[i] = 1'b0;
        current_br_mask  = 20'h0;
        flush_wait_cycles = 4;     // re-arm: ép idle 4 iter để rob_tail_idx update
        `uvm_info("RAND",
          "[E] flush_valid → clear tracking + cooldown 4 iter (chờ rob_tail_idx update)",
          UVM_MEDIUM)
      end
    endfunction
